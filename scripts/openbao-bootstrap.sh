...
# ── 8. Create CA cert ConfigMap for ESO ────────────────────────────

echo ""
echo "=== Creating CA cert ConfigMap for ESO ==="

# ESO needs to trust OpenBao's TLS. Extract the CA cert from the
# openbao-tls secret (generated by setup.sh).
if kubectl get configmap openbao-ca-cert -n "$ESO_NS" &>/dev/null 2>&1; then
  echo "  CA cert ConfigMap already exists in namespace '$ESO_NS'."
else
  CA_BUNDLE=$(kubectl get secret openbao-tls -n "$OPENBAO_NS" \
    -o jsonpath='{.data.tls\.crt}' | base64 -d)

  kubectl create configmap openbao-ca-cert \
    -n "$ESO_NS" \
    --from-literal="ca.crt=$CA_BUNDLE"
  echo "  CA cert ConfigMap created in namespace '$ESO_NS'."
fi
...
# ── 10. Create ESO Kubernetes auth role ───────────────────────────────
...
echo "  CA cert ConfigMap: $ESO_NS/openbao-ca-cert"
echo ""
echo "  Next steps:"
